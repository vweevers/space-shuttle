...
var SpaceShuttle = (function (_Emitter) {
  ...
  _createDecoratedClass(SpaceShuttle, [{
    ...
    value: function replicate() {
      ...
      d.setWritable(Stream.writable({ objectMode: true }, function (data, _, next) {
    ...
    value: function readStream() {
      ...
      var range = _objectWithoutProperties(opts, ['keys', 'values', 'path', 'old', 'erased']);
      ...
      return pump(this.props.readStream(_extends({ keys: true, values: values }, range)), // Always read keys
      through2.obj(function (kv, _, next) {
        var key = values ? kv.key : kv;

        if (!old) {
          // Skip same (older) paths (TODO: skip ahead)
          if (pathEquals(key[0], prev)) return next();else prev = key[0];
        }

        if (key[3] && !erased) next();else if (values && keys) next(null, kv);else if (keys) next(null, key);else next(null, kv.value);
      }));
    }
    ...
    value: function _writePatches(patches, options, next) {
      ...
      patchLoop: for (var i = 0, l = patches.length; i < l; i++) {
        ...
        var ts = _patch$ts === undefined ? timestamp() : _patch$ts;
    ...
    value: function tree(path, done) {
      ...
      pump(this.readStream({ path: path }), Stream.writable({ objectMode: true }, function (_ref4, _, next) {
        var key = _ref4.key;
