  ...
  _createDecoratedClass(SpaceShuttle, [{
      ...
      var incoming = through2.obj(function (data, _, next) {
        if (Array.isArray(data)) {
          ...
          if (writable && validate(data)) {
            ...
            if (!_this5.filter(patch)) return next();
    ...
    value: function batch(patches, options, cb) {
      ...
      var next = function next(err) {
        ...
        if (--_this6.pendingBatches === 0) {
          _this6.future(_this6.emit.bind(_this6, 'drain'));
        }

        if (cb) cb(err);else if (err) _this6.emit('error', err);
      };
